...
# NOTE: `mercury start server` is used here (not `python run.py`) because
# PACKAGE-mode installs have no run.py file at all, and this CLI runner works
# identically in both SOURCE and PACKAGE mode. It does not auto-read .env,
# so load it into the session first.
Write-Host @"
