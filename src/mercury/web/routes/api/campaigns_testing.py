...
def api_send_test_email():
    ...
    try:
        ...
        with session_scope() as session:
            ...
            if not smtp_configs:
                return jsonify(
                    {"success": False, "error": "No active SMTP servers configured"}
                ), 400
        ...
        diagnostics = {
            ...
            "owner_match": (
                "yes"
                if owning_server_name and owning_server_name == result.smtp_server
        ...
        if diagnostics.get("qr_code_referenced_in_body") and not diagnostics.get(
            "qr_code_resolved"
        ):
