...
class ErrorAggregator:
    ...
    def get_summary(self) -> ErrorSummary:
        ...
        transient_count = sum(g.count for g in groups if g.is_transient)
        permanent_count = sum(g.count for g in groups if not g.is_transient)
    ...
    def has_critical_errors(self) -> bool:
        ...
        permanent = sum(g.count for g in self._groups.values() if not g.is_transient)
    ...
    def get_recommendations(self) -> List[str]:
        ...
        auth_errors = [g for g in summary.groups if "authentication" in g.error_type.lower()]
        if auth_errors:
            recommendations.append("⚠️  Authentication errors detected - verify SMTP credentials")

        # Check for connection issues
        conn_errors = [g for g in summary.groups if "connection" in g.error_type.lower()]
        ...
        mailbox_errors = [g for g in summary.groups if "mailbox" in g.error_type.lower()]
        if mailbox_errors:
            count = sum(g.count for g in mailbox_errors)
            recommendations.append(f"📭 {count} invalid mailbox(es) - clean recipient list")
