...
class UserRepository(BaseRepository[User]):
    ...
    def get_by_api_key(self, api_key: str) -> Optional[User]:
        ...
        """
        return (
            self.session.query(User).filter(User.api_key == api_key, User.is_active == True).first()
        )
    ...
    def get_admins(self) -> List[User]:
        """Get all admin users."""
        return self.session.query(User).filter(User.is_admin == True, User.is_active == True).all()
