...
from mercury.engine.error_recovery import ErrorRecoveryManager, RecoveryStrategy
from mercury.exceptions import SMTPConnectionError, SMTPAuthenticationError, TransientSMTPError
...
class TestErrorRecoveryManager:
    ...
    def test_smtp_server_switching(self):
        ...
        manager = ErrorRecoveryManager(available_smtp_servers=["smtp1", "smtp2", "smtp3"])

        error = TransientSMTPError("Server busy")
        decision = manager.decide_recovery(error, current_smtp="smtp1", correlation_id="test-123")
    ...
    def test_max_switches_enforced(self):
        ...
        manager = ErrorRecoveryManager(
            available_smtp_servers=["smtp1", "smtp2", "smtp3"], max_smtp_switches=2
        )
    ...
    def test_tracks_attempted_servers(self):
        ...
        manager = ErrorRecoveryManager(available_smtp_servers=["smtp1", "smtp2"])
    ...
    def test_fallback_chain(self):
        ...
        for _ in range(3):
            decision = manager.decide_recovery(
                error, current_smtp=current_smtp, correlation_id=correlation_id
            )
