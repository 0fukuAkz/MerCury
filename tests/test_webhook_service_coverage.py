...
def test_webhook_registration():
    service = WebhookService()
    config = service.register_webhook("http://reg.com", events=[WebhookEvent.CAMPAIGN_COMPLETED], secret="regsec")
...
def test_webhook_get_stats(webhook_service):
    ...
    delivery = WebhookDelivery(id="d1", webhook_id="w1", event=WebhookEvent.EMAIL_SENT, payload={}, timestamp=datetime.now(), success=True)
