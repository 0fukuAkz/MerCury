...
def test_api_create_campaign_valid(mock_create, client, auth_headers):
    ...
    response = client.post(
        ...
        json={"name": "Test Campaign", "subjects": ["Hello"], "from_names": ["Sales"]},
    )
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_api_preview_template(client, auth_headers):
    response = client.post(
        ...
        headers=auth_headers,
        json={"html": "Hello {{ name }}", "placeholders": {"name": "Alice"}},
    )
    assert response.status_code == 200
    data = json.loads(response.data)
...
def test_api_logs(client, auth_headers, db_session):
    ...
    # Add some logs
    log1 = EmailLog(
        recipient_email="success@example.com", status=EmailStatus.SENT, sent_at=datetime.now(UTC)
    )
    log2 = EmailLog(
...
def test_api_register_webhook(client, auth_headers):
    response = client.post(
        "/api/webhooks",
        headers=auth_headers,
        json={"url": "http://example.com", "events": ["sent"]},
    )
    assert response.status_code == 200
