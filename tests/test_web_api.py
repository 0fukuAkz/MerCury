...
def test_api_test_smtp(client):
    ...
    with patch("mercury.web.routes.api.smtp.SMTPRepository") as MockRepo, patch(
        "mercury.web.routes.api.smtp.SMTPService"
    ) as MockService, patch(
        "mercury.web.routes.api.smtp.run_async", return_value={"success": True, "server": "primary"}
    ):
        repo = MockRepo.return_value
        server = Mock()
        server.name = "primary"
