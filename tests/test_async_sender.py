...
from mercury.engine.async_sender import AsyncEmailSender, BulkSendResult, categorize_smtp_error
...
class TestAsyncEmailSender:
    ...
    async def test_dry_run_mode(self):
        ...
        config = SMTPServerConfig(
            name="test", host="smtp.test.com", port=587, username="test", password="pass"
        )
        pool = SMTPConnectionPool([config])
        sender = AsyncEmailSender(
            connection_pool=pool, default_from_email="sender@test.com", dry_run=True
        )

        result = await sender.send_email(
            recipient="user@test.com", subject="Test", html_body="<p>Test</p>"
        )
    ...
    async def test_send_email_success(self):
        ...
        sender = AsyncEmailSender(connection_pool=pool, default_from_email="sender@test.com")

        result = await sender.send_email(
            recipient="user@test.com", subject="Test Subject", html_body="<p>Test body</p>"
        )
    ...
    async def test_send_email_with_attachments(self):
        ...
        attachments = [
            {"data": b"PDF content", "filename": "document.pdf", "content_type": "application/pdf"}
        ]
    ...
    async def test_send_email_failure_transient(self):
        ...
        mock_conn = AsyncMock()
        mock_conn.send_message = AsyncMock(side_effect=ConnectionError("Connection timeout"))
        ...
        sender = AsyncEmailSender(
            connection_pool=pool, retry_queue=retry_queue, default_from_email="sender@test.com"
        )
    ...
    async def test_rate_limiting(self):
        ...
        sender = AsyncEmailSender(
            connection_pool=pool, rate_limiter=rate_limiter, default_from_email="sender@test.com"
        )
    ...
    async def test_bulk_send_with_progress_callback(self):
        ...
        recipients = [{"email": f"user{i}@test.com", "name": f"User{i}"} for i in range(5)]
