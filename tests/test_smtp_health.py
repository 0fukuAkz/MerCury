...
async def test_smtp_service_check_all_health_success(db_session):
    ...
    service.load_from_database = MagicMock(return_value=[MagicMock(name="test-health-ok")])
    ...
    with patch.object(service, "test_connection", mock_test_conn), patch(
        "mercury.data.database.get_session_direct", return_value=db_session
...
def test_api_smtp_health_routes(client_no_login, db_session):
    ...
    with patch("mercury.web.routes.api.smtp.SMTPService") as MockServiceClass:
        ...
        response = client_no_login.post(
            "/api/smtp/health/check", headers={"X-API-Key": "test_api_key"}
        )
        assert response.status_code == 200
