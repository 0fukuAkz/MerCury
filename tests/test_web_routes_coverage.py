...
def test_create_app_with_explicit_app_context(db_engine):
    ...
    with patch("mercury.web.app.init_db"), patch(
        "mercury.security.auth.UserRepository"
    ) as MockRepo, patch("mercury.web.app.set_app_context") as mock_set_ctx, patch(
        "mercury.data.database.get_session_direct", side_effect=TestSession
    ), patch("mercury.services.smtp_service.get_session_direct", side_effect=TestSession), patch(
        "mercury.services.campaign_service.get_session_direct", side_effect=TestSession
    ), patch("mercury.web.routes.api.get_session_direct", side_effect=TestSession), patch(
...
def test_senders_add_email_service_exception(logged_in_client):
    ...
    with patch(
        "mercury.web.routes.senders.IdentityService.add_email", side_effect=Exception("DB error")
    ):
...
def test_senders_add_name_service_exception(logged_in_client):
    ...
    with patch(
        "mercury.web.routes.senders.IdentityService.add_name", side_effect=Exception("DB error")
    ):
        resp = logged_in_client.post(
            "/senders/names",
            data={
...
def test_settings_post_service_exception(logged_in_client):
    ...
    with patch(
        "mercury.web.routes.settings.SettingsService.update_settings",
...
def test_api_create_job_recurring(client, auth_headers):
    ...
    with patch(
        "mercury.services.scheduler_service.SchedulerService.schedule_recurring",
...
def test_api_create_job_interval(client, auth_headers):
    ...
    with patch(
        "mercury.services.scheduler_service.SchedulerService.schedule_interval",
        return_value=mock_job,
    ):
        resp = client.post(
            "/api/scheduling/jobs",
...
def test_api_create_job_service_exception(client, auth_headers):
    ...
    with patch(
        "mercury.services.scheduler_service.SchedulerService.schedule_once",
...
def test_api_bounce_stats(client, auth_headers):
    ...
    with patch(
        "mercury.services.bounce_service.BounceService.get_bounce_stats",
...
def test_api_list_dead_letters(client, auth_headers):
    ...
    with patch(
        "mercury.services.dead_letter_service.DeadLetterService.get_unresolved",
...
def test_api_retry_dead_letter(client, auth_headers):
    ...
    with patch(
        "mercury.services.dead_letter_service.DeadLetterService.retry_dead_letter",
...
def test_api_discard_dead_letter(client, auth_headers):
    ...
    with patch(
        "mercury.services.dead_letter_service.DeadLetterService.mark_resolved",
        return_value=mock_result,
    ):
...
def test_api_discard_dead_letter_not_found(client, auth_headers):
    ...
    with patch(
        "mercury.services.dead_letter_service.DeadLetterService.mark_resolved", return_value=None
    ):
