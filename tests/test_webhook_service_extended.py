...
class TestWebhookServiceExtended:
    ...
    async def test_deliver_webhook_retry_failure(self, webhook_service):
        ...
        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            delivery = await webhook_service._deliver_webhook(webhook, WebhookEvent.EMAIL_FAILED, {})
    ...
    async def test_convenience_methods(self, webhook_service):
        ...
        webhook_service.notify.assert_called_with(WebhookEvent.EMAIL_SENT, pytest.approx({
        ...
        webhook_service.notify.assert_called_with(WebhookEvent.CAMPAIGN_STARTED, {
            'campaign_id': "c1",
            'campaign_name': "Camp1",
    ...
    async def test_get_delivery_stats(self, webhook_service):
        ...
        webhook_service._deliveries = [
            ...
            WebhookDelivery("3", "w1", WebhookEvent.EMAIL_BOUNCED, {}, datetime.now(), success=False)
        ]
