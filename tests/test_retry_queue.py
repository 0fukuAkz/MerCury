...
class TestRetryQueue:
    ...
    async def test_add_item(self):
        ...
        await queue.add(
            id="test-123", data={"recipient": "test@example.com"}, error="Connection failed"
        )
    ...
    async def test_process_retry_success(self):
        ...
        config = RetryConfig(max_attempts=3, base_delay=0.1, max_delay=0.5, process_interval=0.1)
        ...
        await queue.add(id="test-123", data={"test": "data"}, error="Test error")
    ...
    async def test_process_retry_permanent_failure(self):
        ...
        config = RetryConfig(max_attempts=3, base_delay=0.1, max_delay=0.3, process_interval=0.1)
    ...
    async def test_exponential_backoff(self):
        ...
        config = RetryConfig(max_attempts=4, base_delay=0.1, max_delay=1.0, process_interval=0.1)
        ...
        if len(retry_times) >= 2:
            ...
            for i in range(1, len(delays)):
                # Due to jitter between 0.5x and 1.5x, the minimum theoretical ratio between
                # consecutive delays is roughly 0.66. We use 0.5 for tolerance.
                assert delays[i] >= delays[i - 1] * 0.5  # 50% tolerance

        await queue.stop()

    async def test_max_retries(self):
        """Test max retries enforcement."""
        config = RetryConfig(max_attempts=2, base_delay=0.05, process_interval=0.1)
