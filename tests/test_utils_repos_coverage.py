...
class TestRecipientRepositoryCoverage:
    ...
    def _create_recipient(
        self, session, list_id, email, status="pending", valid=True, suppressed=False
    ):
...
class TestLogRepositoryCoverage:
    ...
    def test_get_smtp_performance_stats(self, db_session):
        ...
        assert s1["failed"] == 1
        assert s1["bounces"] == 0
        assert s1["unique_opens"] == 1
        assert s1["unique_clicks"] == 0
        assert s1["success_rate"] == 50.0
        assert s1["open_rate"] == 100.0
        assert s1["click_rate"] == 0.0

        s2 = next(s for s in stats if s["smtp_server"] == "server2")
        assert s2["total"] == 1
        assert s2["success"] == 0
        assert s2["failed"] == 1
        assert s2["bounces"] == 1
        assert s2["unique_opens"] == 0
        assert s2["unique_clicks"] == 0
        assert s2["success_rate"] == 0.0
        assert s2["open_rate"] == 0.0
        assert s2["click_rate"] == 0.0
...
class TestUserRepositoryCoverage:
    ...
    def _create_user(self, session, username="testuser", email="test@test.com", is_active=True):
        ...
        u = User(username=username, email=email, is_active=is_active, is_admin=False)
    ...
    def test_get_active_users(self, db_session):
        ...
        self._create_user(
            db_session, username="inactive1", email="inactive1@test.com", is_active=False
        )
...
class TestEncryptionCoverage:
    ...
    def test_get_and_set_encryption_service(self):
        ...
        from mercury.security.encryption import (
            EncryptionService,
...
class TestExceptionsCoverage:
    ...
    def test_smtp_exception_details(self):
        ...
        err = SMTPException(
            "test", smtp_server="mail.test.com", smtp_response="550 refused", is_transient=False
        )
