...
class TestDynamicContent:
    ...
    async def test_dynamic_attachment_filename(self, db_engine, db_session, tmp_path, monkeypatch):
        ...
        out = materialized[0]
        assert (
            out["filename"] == "invoices/Alice.pdf"
    ...
    async def test_mixed_rotation_and_substitution(self):
        ...
        mock_sender = Mock()
        mock_sender.send_email = AsyncMock(
            return_value=EmailResult(True, "r", "c", datetime.now(UTC))
        )
        service._sender = mock_sender
    ...
    async def test_from_email_priority(self):
        ...
        config = EmailConfig(from_email="default@test.com", from_emails=["rotate@test.com"])
        service.configure(config)
        service._sender = Mock()
        service._sender.send_email = AsyncMock(
            return_value=EmailResult(True, "r", "c", datetime.now(UTC))
        )
