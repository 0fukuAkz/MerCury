...
def test_docx_generator_from_html():
    ...
    with patch("docx.Document") as MockDocument, patch("io.BytesIO") as MockBytesIO:
        mock_doc = Mock()
...
def test_image_generator():
    ...
    with patch.dict("sys.modules", {"weasyprint": None}), patch("PIL.Image.new") as MockNew, patch(
        "PIL.ImageDraw.Draw"
    ) as MockDraw, patch("PIL.ImageFont.load_default"):
        mock_img = Mock()
