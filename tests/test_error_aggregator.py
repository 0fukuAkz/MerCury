...
class TestErrorAggregator:
    ...
    def test_separates_different_errors(self):
        ...
        aggregator.add_error(
            SMTPConnectionError("Connection failed"), "user1@test.com", is_transient=True
        )
        aggregator.add_error(
            SMTPAuthenticationError("Auth failed"), "user2@test.com", is_transient=False
        )
        aggregator.add_error(
            SMTPMailboxError("Mailbox not found"), "user3@test.com", is_transient=False
        )
    ...
    def test_transient_vs_permanent_counts(self):
        ...
        for i in range(3):
            aggregator.add_error(
                TransientSMTPError("Temp error"), f"user{i}@test.com", is_transient=True
            )

        # Add permanent errors
        for i in range(2):
            aggregator.add_error(
                SMTPAuthenticationError("Auth failed"), f"admin{i}@test.com", is_transient=False
            )
    ...
    def test_top_errors(self):
        ...
        for _ in range(10):
            aggregator.add_error(
                SMTPConnectionError("Connection error"), "user@test.com", is_transient=True
            )

        for _ in range(5):
            aggregator.add_error(
                SMTPAuthenticationError("Auth error"), "user@test.com", is_transient=False
            )

        for _ in range(2):
            aggregator.add_error(
                SMTPMailboxError("Mailbox error"), "user@test.com", is_transient=False
            )
    ...
    def test_critical_errors_detection(self):
        ...
        for _ in range(8):
            aggregator.add_error(
                SMTPAuthenticationError("Auth failed"), "user@test.com", is_transient=False
            )

        # Add few transient
        for _ in range(2):
            aggregator.add_error(
                TransientSMTPError("Temp error"), "user@test.com", is_transient=True
            )
