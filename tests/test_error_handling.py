...
class TestCampaignServiceErrors:
    ...
    @pytest.mark.asyncio
    async def test_run_campaign_db_commit_error(self, service, caplog):
        ...
        mock_result.results = [
            MagicMock(success=True, recipient="test@example.com", server_name="smtp1")
        ]
        ...
        with patch(
            ...
            # The async DB writer now catches exceptions internally to prevent the queue task from crashing
            await service.run_campaign(recipients)

            # The async task should have attempted the write and caught the error, logging it.
            # It could be either the batch flush or the final flush depending on timing
            # The async task should have attempted the write and caught the error
            assert MockLogRepo.return_value.bulk_create.called, "bulk_create was not called"
